...
class CanonicalPositionContractTests(unittest.TestCase):
    ...
    def test_identity_lineage_and_specialist_ownership_are_preserved(self):
        ...
        self.assertEqual(POSITION["canonical_identity"], "gradient-integrity-quorum")
    ...
    def test_capabilities_name_repository_native_quorum_mechanisms(self):
        self.assertEqual(
            CAPABILITIES["capability_family"], "distributed_gradient_integrity_admission"
        )
        capabilities = set(CAPABILITIES["capabilities"])
        self.assertIn("finite-gradient-rank-voting", capabilities)
        self.assertIn("poison-rank-isolation", capabilities)
        self.assertIn("minimum-healthy-ratio-admission", capabilities)
        self.assertIn("deterministic-quorum-receipts", capabilities)
        self.assertNotIn("hyper-scaling", capabilities)

    def test_nvidia_edges_do_not_claim_integration(self):
    ...
    def test_evolution_and_public_boundary_are_material(self):
        self.assertIn("weighted/quorum-authenticated", POSITION["next_evolution"])
